...
| DEP-005 | 2026-05-22 | files: completions/_git-elegant, completions/git-elegant.bash | `eg completion <shell>` | `eg git doctor` / regenerate completion | 2027-01-31 | active |
...
| DEP-011 | 2026-05-24 | former catalog command `repo list` | `memory repositories` (DEP-018 reuses `repo list` for former `repo status`) | use `eg memory repositories` | 2027-01-31 | active |
| DEP-012 | 2026-08-28 | commands: `profile *`, `memory profiles`; shared/per-repo memory keys `profiles`, `profile_id` | `workspace *`, `memory workspaces`, `workspaces`, `workspace_id` | automatic (memory keys); use `workspace` (commands) | 2027-06-30 | active |
