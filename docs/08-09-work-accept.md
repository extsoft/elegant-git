...
The [stash pipe and the branch pipe](reference/pipes.md) wrap the whole run, so uncommitted
changes and the branch you were on are preserved and restored — you end up back where you started.
...
deletes that remote branch. The recorded source for the accepted branch is cleared.
