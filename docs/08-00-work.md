...
- on an idle feature branch with remotes — `start` / `accept` / `list` / `push` / `track` / `quit`
- on an idle feature branch without remotes — `start` / `accept` / `list` / `quit`
...
`start` / `quit`, and `push` and `track` drop out of the idle set.
