...
func TestRecordOncePerID(t *testing.T) {
	...
	SetOutputWriter(&buf)
	Record(DEP002, "personal hook: .workflows/foo", "new/path", "git elegant repo migrate")
...
func TestRecordLegacyCommand(t *testing.T) {
	...
	Flush()
	want := "Warning: the `start-work` command is deprecated; please use `work start`. Run `git elegant git migrate` to migrate automatically.\n"
	if buf.String() != want {
