// Package legacy maps deprecated flat command names to object-first paths and ids.
package legacy
...
var LegacyToID = map[string]cmdid.ID{
	...
	"show-workflows":     {Command: "hook", Action: "status"},
...
func IDFromLegacy(name string) (cmdid.ID, bool) {
	id, ok := LegacyToID[name]
	return id, ok
}
...
func ReplacementCommand(name string) string {
	path, ok := LegacyToPath[name]
	if !ok {
		return ""
	}
...
func AliasValue(name string) string {
	path, ok := LegacyToPath[name]
	if !ok {
		return ""
	}
...
// NewArgPath joins the replacement path for a legacy name.
func NewArgPath(name string) string {
	p, ok := LegacyToPath[name]
	if !ok {
		return ""
	}
	return stringsJoin(p)
}
