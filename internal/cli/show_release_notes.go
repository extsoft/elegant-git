...
func newShowReleaseNotesCommand(spec commandSpec) *cobra.Command {
	c := &cobra.Command{
		Use:   spec.name,
		Short: spec.purpose,
		Long:  spec.purpose,
		RunE: func(cmd *cobra.Command, args []string) error {
			return runWithWorkflows(cmd, func() error {
				return showReleaseNotesRun(args)
			})
		},
	}
...
func showReleaseNotesRun(args []string) error {
...
func formatReleaseNotes(args []string) (string, error) {
	...
	case "smart":
		...
		return simpleReleaseNotes(diapason), nil
	default:
		exitWorkflowError(fmt.Sprintf("A layout can be 'simple' or 'smart'! '%s' layout is not supported.", layout))
		return "", nil
	}
}
