...
func hookNewSpec(commandID, hookType, location *string) argspec.Spec {
	return argspec.Spec{Inputs: []argspec.Input{
		...
		argspec.PositionalInputWithComplete("hook-type", 1, true, "Hook type", hookType, nil, sources.HookTypes, true),
		argspec.PositionalInputWithComplete("location", 2, true, "Hook location", location, nil, sources.HookLocations, true),
	}}
}
